...
import net.neoforged.neoforge.network.PacketDistributor as NeoForgePacketDistributor

@AutoService(PacketDistributor::class)
class NeoForgePacketDistributor : PacketDistributor {
    override fun sendToServer(packet: CustomPacketPayload) {
        if (packet.type().id !in NeoForgeNetworkState.registeredServerboundPackets) {
            throw IllegalStateException("Packet payload with ID ${packet.type().id} is not registered for serverbound (C2S) sending")
        }
        NeoForgePacketDistributor.sendToServer(packet)
    }
    ...
    override fun sendToPlayer(
        ...
        if (packet.type().id !in NeoForgeNetworkState.registeredClientboundPackets) {
            throw IllegalStateException("Packet payload with ID ${packet.type().id} is not registered for clientbound (S2C) sending.")
        }
        NeoForgePacketDistributor.sendToPlayer(player, packet)
    }
}